...
rctrl1 <- trainControl(method = "cv", number = 10, repeats = 10, returnResamp = "all")

# Make a custom tuning grid
tuneGrid <- expand.grid(lambda = 10^seq(10, -2, length=100))
...
#KNN
set.seed(849)
m.knn <- train(votos ~ total_receita * total_despesa * recursos_de_pessoas_juridicas, data = treino, method = "knn", 
               tuneGrid = data.frame(k = 10:31),
               preProc = c("center", "scale"), tuneLength = 10)
```
